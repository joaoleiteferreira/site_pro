...
<!-- Structure Begin -->
		<div class="wrapper">
			<div class="grid_24"><h2>SandBox Performance Tests</h2>
...
//         require $KoolControlsFolder."../../KoolCalendar/koolcalendar.php";
	...
	$ds->InsertCommand = "insert into Testsconfig (Name,File,Class,Notes,Platform,ExecServer,Testbed,Date,Priority,Project,Time,tree) values ('@Name','@File','@Class','@Notes','@Platform','@ExecServer','@Testbed','@Date','@Priority','$project','@Time','@tree')";
 	$ds_detail = new MySQLDataSource($db_con);
	$ds_detail->SelectCommand = "select ID,RESULT,MODEL,RUN_PARAMETERS,SUBMITTER,BANDWIDTH,XML_FILE,TIME_FINISH,DURATION,NOTES,TRAFFIC,CONFIG,VERSION,LIMIT_FACTOR,LOG,STATS from resuts_performance WHERE CLASS='SANDBOX' order by TIME_FINISH desc";
	$ds_detail->DeleteCommand = "delete from RESULTS where ID=@ID AND End_Time='@End_Time' AND Tree='@Tree' AND Result='@Result'";
...
// 	$grid->MasterTable->AddColumn($col);
	...
	$table_order_detail->DataSource = $ds_detail;
	$table_order_detail->AddRelationField("ID","ID");
	$table_order_detail->AllowEditing = false;
	$table_order_detail->AllowSorting = true;
	$table_order_detail->AllowFiltering = false;
	...
	$column->Width = "50px";
	$column->Align = "left";
	$table_order_detail->AddColumn($column);
...
$table_order_detail->AddColumn($column);
	...
	$table_order_detail->DisableAutoGenerateDataFields = "ID";//Disable generate column for orderNumber data fields.
