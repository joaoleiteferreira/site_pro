    ...
    jQuery.get('/tmp/test_cvs', function(data) {
	 ...
	 time_ti = "";
      ...
      jQuery.each(lines, function(lineNo, line) {
		...
		if ( item.indexOf("uut:CPU") !== -1  ) {
			options.series[itemNo -2] = ({name: item ,type: 'column', data: [], color: 'rgba(0,0,255, 0.5)', tooltip: { valueSuffix: ' %'},visible: true ,dataGrouping: {enabled: true, groupPixelWidth: 50 , approximation: "average"}});
		} else if ( item.indexOf("uut:QFP") !== -1) {
			options.series[itemNo -2] = ({name: item ,type: 'column', data: [], color: 'rgba(100,10,100, 0.5)', tooltip: { valueSuffix: ' %'},visible: true ,dataGrouping: {enabled: true, groupPixelWidth: 50 , approximation: "average"}});
		} else if ( item.indexOf("uut:iCPU") !== -1) {
			options.series[itemNo -2] = ({name: item ,type: 'column', data: [], color: 'rgba(100,100,255, 0.5)', tooltip: { valueSuffix: ' %'},visible: true ,dataGrouping: {enabled: true, groupPixelWidth: 50 , approximation: "average"}});
		} else if ( item.indexOf("CPU") !== -1) {
			options.series[itemNo -2] = ({name: item ,type: 'column', data: [], tooltip: { valueSuffix: ' %'},visible: false ,dataGrouping: {enabled: true, groupPixelWidth: 50 , approximation: "average"}});
		} else if ( item.indexOf("QFP") !== -1) {
			options.series[itemNo -2] = ({name: item ,type: 'column', data: [], tooltip: { valueSuffix: ' %'},visible: false ,dataGrouping: {enabled: true, groupPixelWidth: 50 , approximation: "average"}});
		} else if ( item.indexOf("Upload") !== -1 || item.indexOf("Download") !== -1) {
			options.series[itemNo -2] = ({name: item , yAxis: 1, data: [], tooltip: { valueSuffix: ' mbps'}, visible: true ,dataGrouping: {enabled: true, groupPixelWidth: 50 , approximation: "average"} });
		} else if ( item.indexOf("SYN_ACK") !== -1 || item.indexOf("response_time") !== -1) {
			options.series[itemNo -2] = ({name: item ,yAxis: 2, data: [], tooltip: { valueSuffix: ' ms'},visible: false ,dataGrouping: {enabled: true, groupPixelWidth: 50 , approximation: "average"}});
		} else {
