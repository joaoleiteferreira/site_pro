...
function graph(MODEL,CONFIG,ID) {
  jQuery(document).ready(function() {
	...
	var lineNo;
        var options;
    graph_title = MODEL + " config:" + CONFIG;
    var options = {
      chart: {
              type: 'column',
        renderTo: 'container2',
	...
	var array_data =[];
	file2 = "/site_pro/mysql_query/by_version.php?ID=" + ID;
...
<!-- Structure Begin -->
		<div class="wrapper">
			<div class="grid_24"><h2>Analyse Sandbox performance on specific version</h2>
...
//         require $KoolControlsFolder."../../KoolCalendar/koolcalendar.php";
	...
	$ds->SelectCommand = "SELECT ID,CONFIG,MODEL,count(DISTINCT VERSION) as HITS,TRAFFIC FROM resuts_performance WHERE RESULT='PASS' and CLASS='SANDBOX' GROUP BY CONFIG,TRAFFIC,MODEL order by HITS desc";
...
// 	$grid->MasterTable->AddColumn($col);
	...
	$column = new GridBoundColumn();
	$column->HeaderText = "Number of version run";
	...
	$column->OnClick = "graph(\"{MODEL}\",\"{CONFIG}\",\"{ID}\")";
